...
args = commandArgs(T)
source          = args[1] = "API"
api_token       = args[2] = hhs_baseline_moz
study_area_1_id = args[3] = "massinga"
study_area_1    = args[4] = "Massinga"
...
for(i in 1:length(special_characters)) {
  ...
  hhs_data$his_hh_other = gsub(c, r, hhs_data$his_hh_other)       #HQ40.1
...
#Put all clustar names in same column
#test moz. Check if something besides locality can be NA.
hhs_data$cluster[!is.na(hhs_data$locality)] = 
  rowSums(hhs_data[!is.na(hhs_data$locality), grepl("cluster_", names(hhs_data))], na.rm = T)
...
if(nrow(rerecorded_hh_summary) > 0) {
  ...
  current_cluster = rerecorded_hh_summary$cluster
