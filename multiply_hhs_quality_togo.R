  ...
  if(length(number_hh_selected_visited) > 0) {
    ...
    if(ncol(trial_profile_checked) > maximum_number_of_columns) {
      ...
      print(kable(trial_profile_checked[,1:(middle + 2)], "html", escape = F) %>%
              ...
              add_indent(c(7, 10), level_of_indent = 4) %>%
              add_indent(c(8, 11, 12, 13), level_of_indent = 5) %>%
              add_indent(c(9), level_of_indent = 6) %>%
              add_indent(c(9, 11, 12, 13, 16, 17), level_of_indent = 4)
